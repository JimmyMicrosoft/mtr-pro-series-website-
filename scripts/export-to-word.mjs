/**
 * Export MDX lesson content to Word (.docx) files — one per track.
 *
 * Usage:
 *   node scripts/export-to-word.mjs              # export all 12 tracks
 *   node scripts/export-to-word.mjs mtr-foundations mtr-ecosystem   # one track only
 *
 * Output: exports/word/[program-slug]--[track-slug].docx
 */
...
const OUTPUT_DIR = resolve(__dirname, '../exports/word')
...
function videoPlaceholder(lessonId, idx, suggestion = '') {
  const children = [
    new Paragraph({
      children: [
...
function imagePlaceholder(lessonId, idx, suggestion = '') {
  const children = [
    new Paragraph({
      children: [
...
function calloutBox(type, bodyLines) {
  const styles = {
...
function knowledgeCheckBox(kcLines) {
  const children = [
    new Paragraph({
  ...
  for (const raw of kcLines) {
    ...
    // Question opening (content-style)
    ...
    // Plain content (question text lines, explanation continuation)
    if (l) {
      children.push(new Paragraph({
...
function parseBody(mdx, lessonId) {
  const elements = []
  ...
  // Helper: collect multi-line component until closing tag or />
  function collectComponent(startLine, closingTag) {
    let text = startLine
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('## ')) {
      ...
      // Only add generic image placeholder after H2 if there are no MEDIA comments in this lesson
      if (!hasMediaComments) {
        imgCount++
...
const trackTitle  = (s) => TRACK_TITLES[s]   ?? s.split('-').map(cap).join(' ')
const programTitle = (s) => PROGRAM_TITLES[s] ?? s.split('-').map(cap).join(' ')

// ── Quiz section builder ──────────────────────────────────────────────────────

function buildQuizSection(quizData) {
  ...
  children.push(
    ...
    new Paragraph({
      children: [
        new TextRun({ text: '🧠  TRACK QUIZ', bold: true, color: '7C3AED', size: 32 }),
      ],
  ...
  for (const q of questions) {
    ...
    children.push(
      new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        borders: { top: borderConfig, bottom: borderConfig, left: borderConfig, right: borderConfig, insideH: noBorder, insideV: noBorder },
        rows: [new TableRow({
          children: [new TableCell({
...
async function buildTrackDocument(programSlug, tSlug, lessons) {
  const children = []
  ...
  for (let li = 0; li < lessons.length; li++) {
    ...
    // Page break before each lesson
    if (li > 0) {
      children.push(new Paragraph({ children: [new PageBreak()] }))
    }

    // Lesson ID marker (used by import script)
    ...
    const objParagraphs = (lesson.objectives ?? []).length > 0 ? [
    ...
    // Video placeholder (lesson intro)
  ...
  // Quiz section
  try {
    const quizPath = join(resolve(__dirname, '../content/programs', programSlug, 'tracks', tSlug, 'quiz.json'))
...
async function main() {
  ...
  const [filterProgram, filterTrack] = process.argv.slice(2)

  const programSlugs = ls(CONTENT_DIR).filter((f) => isDir(join(CONTENT_DIR, f)))
  let trackCount = 0

  for (const programSlug of programSlugs) {
    if (filterProgram && programSlug !== filterProgram) continue

    const tracksDir = join(CONTENT_DIR, programSlug, 'tracks')
    const trackSlugs = ls(tracksDir).filter((f) => isDir(join(tracksDir, f)))

    for (const tSlug of trackSlugs) {
      if (filterTrack && tSlug !== filterTrack) continue

      process.stdout.write(`  Building: ${programSlug} / ${tSlug} ...`)

      // Collect all lessons for this track
      const lessons = []
      const modulesDir = join(tracksDir, tSlug, 'modules')
      ...
      for (const mSlug of moduleSlugs) {
        ...
        for (const file of lessonFiles) {
          ...
          if (!data.id) continue
          lessons.push({ ...data, body: content.trim() })
        }
      }

      if (!lessons.length) { console.log(' (no lessons, skipped)'); continue }

      lessons.sort((a, b) => (a.module - b.module) || (a.lessonIndex - b.lessonIndex))

      const doc = await buildTrackDocument(programSlug, tSlug, lessons)
      const buffer = await Packer.toBuffer(doc)
      const outFile = join(OUTPUT_DIR, `${programSlug}--${tSlug}.docx`)
      writeFileSync(outFile, buffer)

      console.log(` ✓  ${lessons.length} lessons → ${outFile.replace(OUTPUT_DIR + '/', '')}`)
      trackCount++
    }
  }

  console.log(`\n✅ Exported ${trackCount} track documents to ${OUTPUT_DIR}\n`)
}
